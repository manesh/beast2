...
You may emit multiple independent tool calls in a single response when they can be executed in parallel.

Emit no more than **10 tool calls** in a single response.

Avoid duplicate or unnecessary tool calls.

Only request information that meaningfully advances the current objective.

---

# Output Contract

Your next response must contain exactly one of the following:

- One `b2_final`
- One or more `b2_tool_call` blocks (maximum 10)

Do not emit markdown code fences.

Do not emit explanations of the protocol.

Do not output free-form JSON.
