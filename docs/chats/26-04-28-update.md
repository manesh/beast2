implemented bare minimum llm support and docs browser. first test with 20b param oss gpt model failing at one-shot cdi editing, adds variants but fails at copy pasting below.

difficulty of implementing patching behavior?
...
Not hard if you avoid general diff patching first.

For CDI, use structured patch ops, not "rewrite whole document."

best v0.1 approach

Make the LLM output tiny commands like:

$patch: add-param-value
$target-param: goblin-attire
$value: red armor

or JSON if easier internally:
...
Then Beast applies it with your parser.

why this fixes your failure

One-shot rewrite asks the LLM to:
...
20B models often fail at "copy exact large text."
...
That handles most CDI edits.

flow

context + document + instruction
→ llm outputs patch ops
→ beast validates target exists
→ beast applies patch
→ beast shows diff
→ user accepts
...
structured patching: medium-low
general text diff patching: medium-high
llm full-doc rewrite: unreliable

best next move

Do this before trying smarter prompts:

llm edits intent
beast edits text

That is much more Beast.

Final law:

llm proposes patch
parser applies patch
user approves diff
...
technical details c backend for finding patch section then replacing patch
